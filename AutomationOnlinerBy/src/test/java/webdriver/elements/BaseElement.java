...
public abstract class BaseElement extends BaseEntity {
	...
	public Label clickAndWait() {
		...
		if (browser.getDriver() instanceof JavascriptExecutor) {
			((JavascriptExecutor)browser.getDriver()).executeScript("return document['readyState'] ? 'complete' == document.readyState : true", element);
		}
	...
	/**
	 * Click on the item CheckBox.
	 */
	public void check() {
		waitForIsElementPresent();
		info(getLoc("loc.checkbox"));
		browser.getDriver().getMouse().mouseMove(element.getCoordinates());
		if (browser.getDriver() instanceof JavascriptExecutor) {
			((JavascriptExecutor)browser.getDriver()).executeScript("arguments[0].style.border='3px solid red'", element);
		}
	...
	/**
	 * Take results by Text from locator and pass to String
	 * @return
	 */
	...
	/**
	 * Take results by Attribute from locator and pass to String
	 * @return
	 */
	...
	public void eventElement() {
		...
		WebElement moveOnMenu = browser.getDriver().findElement(locator);
